...
| 16. Rendering | Quick Montage реализован, advanced rendering в плане |
...
Отдельный Quick Montage vertical slice создаёт готовый MP4 до реализации
AI-стадий.
...
Quick Montage является временным рабочим маршрутом до полного AI pipeline:
...
build_quick_montage_plan
    |
    +--> chronological asset ordering
    +--> centered video excerpts
    +--> configurable photo duration
    |
    v
quick_timeline.json
    |
...
rate и channel layout. Это позволяет объединить их через concat demuxer без
повторного кодирования итогового файла.

Текущие ограничения:

- нет semantic scene detection;
- нет quality ranking и duplicate removal;
- нет переходов, музыки, titles и subtitles;
