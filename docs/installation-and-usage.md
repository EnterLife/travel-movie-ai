...
- находит почти одинаковые сцены и оставляет лучший вариант;
- создаёт мультимодальные описания для следующего Story Builder;
- опционально распознаёт речь через Faster Whisper;
- строит opening/journey/highlight/finale storyboard;
...
10. Выберите музыкальный режим: `AI Auto`, генерация, библиотека, ручной файл
   или без музыки.
11. Для первого прохода включите `Быстрый preview`.
12. Нажмите `Запустить AI-монтаж`.
13. В галерее отметьте сцены как `Авто`, `Обязательно` или `Исключить`.
14. Повторите монтаж: неизменённые AI-результаты будут взяты из кэша.
15. Отключите preview и соберите финальный MP4.
...
quality score, motion, camera shake, noise, technical rejection reasons и
backend (`opencv` или `pillow`). Метрики участвуют в ranking.

### `artifacts/speech_analysis.json`

Создаётся при включённом Faster Whisper. Содержит scene transcript, language,
confidence, provider/model и cache metadata.

### `artifacts/duplicates.json`

Группы визуально похожих сцен, keeper и similarity. Исходные файлы не
...
Детерминированные секции opening, journey, highlight и finale с event/scene IDs.

### `artifacts/selection_decisions.json`

Для каждой сцены сохраняет итоговый score, факт выбора и объяснение: ручное
решение, техническая проблема, duplicate, duration budget или event diversity.
...
Текущий semantic montage ещё не выполняет embedding-based similarity и
audio classification. Transcript влияет на multimodal descriptions при
включённом Faster Whisper.
