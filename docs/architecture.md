...
| 3. Frame Sampling | Representative midpoint frame реализован |
| 4. Vision Analysis | LM Studio/Qwen flow реализован |
...
| 16. Rendering | Переходы, музыка и ducking реализованы |
...
Scene Detection (semantic mode)
    ...
    +--> LM Studio structured vision analysis
    +--> importance and diversity ranking
    |
    v
quick_timeline.json
    |
...
QuickMontageRenderer
    ...
    +--> local music selection and ducking
    |
    v
final.mp4
```
...
- нет event-aware Story Builder, titles и subtitles;
