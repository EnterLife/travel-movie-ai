...
- извлекает representative frame;
- анализирует кадр локальной vision-моделью через LM Studio;
- валидирует caption, activity, location, emotion, tags и importance;
- выбирает наиболее важные и разнообразные сцены;
- добавляет переходы, локальную музыку и ducking;
...
Семантический режим требует запущенный локальный LM Studio с vision-моделью.
...
3. Устанавливает базовые зависимости TravelMovieAI.
4. Запускает `main.py`.
...
6. Настройте длительность, стиль истории и переход.
7. Оставьте `Семантический AI-отбор` включённым для LM Studio или отключите для
   быстрого режима.
8. При необходимости укажите локальный музыкальный файл. Без пути приложение
   ищет soundtrack по музыкальному имени файла и в `assets/music`.
9. Нажмите `Собрать фильм`.
10. Просмотрите или скачайте MP4.
...
TRAVELMOVIEAI_VISION_MODEL=qwen2.5-vl-7b-instruct
TRAVELMOVIEAI_VISION_TIMEOUT_SECONDS=120
```

Если локальный сервер или модель недоступны, semantic job завершится явной
ошибкой. Автоматического перехода в облако нет.
...
- `TRAVELMOVIEAI_VISION_MODEL`: identifier загруженной vision-модели.
...
Таблица `scenes` хранит временные границы, representative frame, vision
...
Границы сцен, detector/cache metadata и пути representative frames. Если
...
Структурированный результат локальной vision-модели: caption, location,
activity, emotion, people count, tags, importance score, provider, model и
prompt version.
...
Текущий semantic montage ещё не удаляет дубли, не учитывает transcript и
quality metrics, не строит события и полноценный storyboard.
...
- identifier модели совпадает с `TRAVELMOVIEAI_VISION_MODEL`.
