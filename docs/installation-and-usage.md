...
- позволяет выбрать загруженную модель LM Studio в интерфейсе;
...
6. Выберите vision-модель из списка моделей LM Studio.
7. Настройте стиль, длительность, переходы и `Auto/CUDA/CPU`.
8. Оставьте semantic и OpenCV analysis включёнными для лучшего отбора.
9. Выберите музыкальный режим: `AI Auto`, генерация, библиотека, ручной файл
   или без музыки.
10. Нажмите `Запустить AI-монтаж`.
11. Просмотрите или скачайте MP4.
...
Текущий semantic montage ещё не удаляет дубли, не учитывает transcript, не
строит события и полноценный storyboard.
